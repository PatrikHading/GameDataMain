...
public class GameDataControllerTest {
    ...
    public void testAddPlayer() throws Exception {
        ...
        mockMvc.perform(post("/addPlayers")
                        .param("firstname", firstName)
                        .param("lastname", lastName)
                        .param("jerseynumber", String.valueOf(jerseyNumber)))
                .andExpect(status().is3xxRedirection())
                .andExpect(view().name("redirect:/addPlayers"));

        ArgumentCaptor<Player> playerCaptor = ArgumentCaptor.forClass(Player.class);
        verify(playerService, times(1)).savePlayer(playerCaptor.capture());

        Player capturedPlayer = playerCaptor.getValue();

        assert capturedPlayer.getName().equals(firstName);
        assert capturedPlayer.getLastname().equals(lastName);
        assert capturedPlayer.getJerseynumber() == jerseyNumber;
    }

    @Test
    public void testShowUpdateForm() throws Exception {
        Integer playerId = 1;
        Player player = new Player("John", "Doe", 99);

        when(playerService.getPlayerById(playerId)).thenReturn(player);

        mockMvc.perform(get("/updatePlayers/{id}", playerId))
                .andExpect(status().isOk())
                .andExpect(view().name("updatePlayers"))
    ...
    public void testUpdatePlayer() throws Exception {
        ...
        String updatedLastName = "Doey";
        int updatedJerseyNumber = 10;

        Player updatedPlayer = new Player(updatedFirstName, updatedLastName, updatedJerseyNumber);

        mockMvc.perform(post("/updatePlayers/{id}", playerId)
                        .flashAttr("player", updatedPlayer))
                .andExpect(status().is3xxRedirection())
                .andExpect(view().name("redirect:/registeredPlayers"));

        verify(playerService, times(1)).updatePlayer(eq(playerId), any(Player.class));
    }

    @Test
    public void testAddToTeam1() throws Exception {
        Integer playerId = 1;
        Player player = new Player("John", "Doe", 99);
        ...
        mockMvc.perform(post("/addToTeam1")
        ...
        assert team1Players != null;
        assert team1Players.size() == 1;
        assert team1Players.contains(player);
    }
